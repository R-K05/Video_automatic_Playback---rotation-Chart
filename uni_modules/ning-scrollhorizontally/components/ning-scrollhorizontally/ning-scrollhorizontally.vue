<template>
	<view class="u-swiper" :style="{
		...
		<swiper  :style="{
				height: $u.addUnit(height),
			}" @change="change" :circular="circular" :interval="interval" :duration="duration" :current="current"
			:currentItemId="currentItemId" :previousMargin="130" :nextMargin="130" :acceleration="acceleration"
			:displayMultipleItems="1" :easingFunction="easingFunction">
			<!-- 	:previousMargin="$u.addUnit(previousMargin)" -->
			<swiper-item  v-for="(item, index) in list" :key="index">
				<view  :style="[itemStyle(index)]">
					<image
						v-if="getItemType(item) === 'image'"
						:src="getSource(item)"
						:mode="imgMode"
						@tap="clickHandler(index)"
						style="height: 131px;width:76px;"
					></image>
					<video  v-if="getItemType(item) === 'video'"
						...
						style="width: 76px;height: 131px;" @tap="clickHandler(index)" :show-fullscreen-btn="false" :controls="false" :show-play-btn="false" ></video>
						<!-- :show-center-play-btn="false" 是否展示视频中间播放按钮 -->
